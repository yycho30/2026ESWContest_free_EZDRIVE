...
class InattentionDetector:
    ...
    def update(self, detections, head_yaw, now=None):
        ...
        # ---- 3. decide ----
        # The warning is a sustained state, not a one-shot event: as long as an
        # obstacle past the reaction window is still unconfirmed, this keeps
        # returning 1. It clears by itself once the driver looks at the obstacle
        # (step 2 above) or the obstacle stops being tracked (step 4).
        state = 0
        offenders = []
        for obj in self.tracks.values():
            if not obj.needs_check() or obj.pending_since is None:
                continue
            if (now - obj.pending_since) >= REACTION_TIME_SEC:
                state = 1
